...
class ProxyListScraper:
    ...
    def save_residential_simple(self, residential_proxies):
        """保存家宽代理为简单格式 IP:端口"""
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            timestamp = self.get_cn_time().strftime('%Y%m%d_%H%M%S')
            filename = f"residential_{timestamp}.txt"
            filepath = os.path.join(script_dir, filename)
            ...
            print(f"家宽代理已保存到 {filepath}")
            return True
            
        except Exception as e:
            print(f"保存家宽代理文件错误: {e}")
            return False
...
def main():
    ...
    if proxies:
        ...
        scraper.save_residential_simple(residential_proxies)
